...
- [~] **Phase 4: Inference engine** — slices 4.1 (R1 transitive exposure) + 4.2 (R2 + recursive R1 + fixpoint) + 4.3 (pluggable benchmarkable engines: naive/semi-naive/native + metrics + UI) + 4.4 (embedding severity imputation — latent engine E1: kNN + leave-one-out eval) + pluggable rule pipeline (runtime-editable ordered RuleRegistry, run-rules, Rules UI card) shipped; Phase 6 portfolio next.
...
| 4. Inference engine| 5/N   | Slices 4.1–4.4 + rule pipeline done | 2026-06-13 |
